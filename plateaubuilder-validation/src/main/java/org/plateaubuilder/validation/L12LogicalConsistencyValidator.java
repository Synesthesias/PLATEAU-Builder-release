...
public class L12LogicalConsistencyValidator implements IValidator {
  ...
  public List<ValidationResultMessage> validate(CityModelView cityModelView)
    ...
    for (int i = 0; i < buildings.getLength(); i++) {
      ...
      for (L11LogicalConsistencyValidator.LODInvalid lodInvalid : lodInvalids) {
        for (String polygonId : lodInvalid.getPolygon()) {
          elementErrors.add(new GmlElementError(buildingID, null, polygonId, tagLOD23s.toString(), "LOD23", 0));
        }
      }
  ...
  public List<L11LogicalConsistencyValidator.LODInvalid> getInvalidLOD(List<Node> tagLOD,
    ...
    for (int i = 0; i < tagPoslists.getLength(); i++) {
      ...
      try {
        ...
        List<Point3D> point3Ds = ThreeDUtil.createListPoint(posString);
          if (this.isPoslistValid(point3Ds)) {
              continue;
          }
